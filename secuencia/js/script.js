class Script {
  ...
  fromJHF(data) {
    ...
    for (let i = 0; i < dataSortedAsGlyphs.length; i++) {
      ...
      if (!hersheyCodeToLetter[characterCode] && pairs == "1") {
        print("missing characterCode: " + characterCode)
        // continue;
      } 
      ...
      if (coordinates.length > 0) {
        ...
        for (let i = 0; i < coordinates.length; i += 2) {
          ...
          if (coordinates.charAt(i).match(" ") && coordinates.charAt(i + 1).match("R")) {
            ...
            let xPosition = this.decodeHersheyCharToCoordinate(coordinates.charAt(i)) + abs(left);
  ...
  decodeHersheyCharToCoordinate(character) {
    ...
    let coordinate = map(value, -hersheyScale, hersheyScale, -1, 1);
    return coordinate;
  }
