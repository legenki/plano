class GlyphEditor {
  ...
  setPathPosition(path) {
    ...
    this.activePath.updatePosition(mouseX - pmouseX, mouseY - pmouseY);
  ...
  combinePaths(path1, path2, anchor) {
    ...
    let newPath = new PathButton(0, 0, this.scriptStrokeWeight * 1.5, false, false, min(path1.index, path2.index));
  ...
  addAnchor() {
    ...
    let xPosition = this.gridify(this.xInsideBounds(mouseX));
    let handleToPrev_xPosition = xPosition;
    let handleToNext_xPosition = xPosition;
    let yPosition = this.gridify(this.yInsideBounds(mouseY));
    ...
    if (this.activeAnchor.handleToNext.isPressed == true || this.activeAnchor.handleToPrev.isPressed == true) {
      ...
      if (this.activeAnchor.angular == true) {
        ...
        if (this.activeAnchor.handleToPrev.isPressed == true && this.activeAnchor.handleToPrev.locked == false) {
          this.activeAnchor.handleToPrev.updatePosition(this.gridify(this.xInsideBounds(mouseX)), this.gridify(this.yInsideBounds(mouseY)));
        } else if (this.activeAnchor.handleToNext.isPressed == true && this.activeAnchor.handleToNext.locked == false) {
          this.activeAnchor.handleToNext.updatePosition(this.gridify(this.xInsideBounds(mouseX)), this.gridify(this.yInsideBounds(mouseY)));
        } else {
          this.activeAnchor.updatePosition(this.gridify(this.xInsideBounds(mouseX)), this.gridify(this.yInsideBounds(mouseY)));
        }

      } else {
        this.activeAnchor.updateBothHandlePosition(this.gridify(this.xInsideBounds(mouseX)), this.gridify(this.yInsideBounds(mouseY)), false);
      }
  ...
  setHandlePosition(anchor) {
    ...
    if (this.activeAnchor.first == true) {
      this.activeAnchor.updateBothHandlePosition(this.gridify(this.xInsideBounds(mouseX)), this.gridify(this.yInsideBounds(mouseY)), true);
    } else {
      this.activeAnchor.updateBothHandlePosition(this.gridify(this.xInsideBounds(mouseX)), this.gridify(this.yInsideBounds(mouseY)), false);
    }
  ...
  editGuides() {
    ...
    if (this.baselineButton.isPressed == true) {
      let shift = this.gridify(this.yInsideBounds(mouseY)) - this.baselineButton.position.y;
      this.baselineButton.updatePosition(this.gridify(this.position.x), this.gridify(this.yInsideBounds(mouseY)));
      ...
      let yPosition = min(this.yInsideBounds(mouseY), this.baselineButton.position.y - this.gridSize);
      ...
      let yPosition = min(this.yInsideBounds(mouseY), this.xHeightButton.position.y - this.gridSize);
      ...
      let yPosition = max(this.yInsideBounds(mouseY), this.baselineButton.position.y + this.gridSize);
      ...
      let xPosition = min(this.xInsideBounds(mouseX), this.rightBoundingButton.position.x - this.gridSize);
      ...
      let xPosition = max(this.xInsideBounds(mouseX), this.leftBoundingButton.position.x + this.gridSize);
  ...
  displayGrid() {
    ...
    for (let x = 0; x < this.width; x += this.gridSize) {
      line(this.position.x + x, this.position.y, this.position.x + x, this.position.y + this.height);
    }

    for (let y = 0; y < this.height; y += this.gridSize) {
      line(this.position.x, this.position.y + y, this.position.x + this.width, this.position.y + y);
    }
  ...
  displayGuides() {
    ...
    strokeWeight(interfaceStrokeWeight);
    stroke(this.displayInfo == true ? glyphEditor_guideColor : gridColor);

    // x-height
    line(this.position.x, this.xHeight, this.position.x + this.width, this.xHeight);

    // ascender height
    line(this.position.x, this.ascenderHeight, this.position.x + this.width, this.ascenderHeight);

    // descender height
    line(this.position.x, this.descenderHeight, this.position.x + this.width, this.descenderHeight);

    // baseline
    line(this.position.x, this.baseline, this.position.x + this.width, this.baseline);

    // left bounding
    line(this.leftBounding, this.position.y, this.leftBounding, this.position.y + this.height);

    // right bounding
    line(this.rightBounding, this.position.y, this.rightBounding, this.position.y + this.height);
  ...
  relativeXPosition(absoluteXPosition) {
    return int(absoluteXPosition * (this.width * 0.9) + this.leftBounding);
  }

  relativeYPosition(absoluteYPosition) {
    return int(absoluteYPosition * (this.width * 0.9) + this.baseline);
  }
  ...
  xInsideBounds(xPosition) {
    return int(constrain(xPosition, this.position.x, this.position.x + this.width));
  }

  yInsideBounds(yPosition) {
    return int(constrain(yPosition, this.position.y, this.position.y + this.height));
  }
  ...
  gridifySegments(value) {
    return round(value / (this.gridSize * this.gridsPerSegment)) * (this.gridSize * this.gridsPerSegment);
  }
